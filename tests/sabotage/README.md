...
| `engine_r8_held_clock.py` | 8 | `fix/report-test-clock`: the report suite's held clock undone ten ways — the hold deleted or taken at the real time; the golden's scrubs keyed on the machine's date or on a shape again; the cover or the footer reading the machine's clock around the hold, always or only for FP8 weights or only on PCIe (the cold check's survivor was the FP8 gate, invisible to a one-config check); and the cover read around the hold with the every-build check unregistered. The checks that compare two builds would notice only when a run crosses a minute; these have to be caught every time |
