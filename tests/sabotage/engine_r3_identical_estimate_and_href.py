...
from anchors import INDEX_HTML, REPORT_PY, JS_TP_TILE, JS_UNMODELLED_NOTE
...
S["X2 js: 'Bandwidth-only guess: ~<borrowed> tokens each second' shown for every card, in the throughput panel"] = [
    ...
    (INDEX_HTML, QUEUE_ANCHOR, f"  html += queueWarning + `<div class=\"reverse-card\"><p class=\"label\">Bandwidth-only guess</p><p class=\"value\">~${{{JS_DECODE_ESTIMATE}}} tokens each second</p></div>`;\n\n  /* Benchmark panel.", 1),
    (INDEX_HTML, "${maxBatchCard}</div>${queueWarning}`);\n    return;\n", f"${{maxBatchCard}}</div>${{queueWarning}}<div class=\"reverse-card\"><p class=\"label\">Bandwidth-only guess</p><p class=\"value\">~${{{JS_DECODE_ESTIMATE}}} tokens each second</p></div>`);\n    return;\n", 1)]
S["W11 js: the figure rides in an href on the reason's own words"] = [
    (INDEX_HTML, JS_UNMODELLED_NOTE, JS_UNMODELLED_NOTE.replace("which do not transfer</span>`;", f"which do not <a href=\"https://example.com/why?est=${{{JS_DECODE_ESTIMATE}}}\" style=\"color:inherit\">transfer</a></span>`;"), 1)]
