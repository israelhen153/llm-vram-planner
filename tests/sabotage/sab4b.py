...
S["R1 js: throughput tile prints '~<borrowed> tok/s per user' only for single-device boards (every catalog card)"] = [
    (INDEX_HTML, JS_TP_TILE, JS_TP_TILE + f"${{state.gpuDevices === 1 ? '<p class=\"sub\">~' + {JS_DECODE_ESTIMATE} + ' tok/s per user (rough)</p>' : ''}}", 1)]
S["R2 js: the same leak only when the card has an FP8 KV cache selected"] = [
    (INDEX_HTML, JS_TP_TILE, JS_TP_TILE + f"${{state.kvBytesPerValue < 2 ? '<p class=\"sub\">~' + {JS_DECODE_ESTIMATE} + ' tok/s per user (rough)</p>' : ''}}", 1)]
S["R3 js: the same leak only for MLA or SWA attention"] = [
    (INDEX_HTML, JS_TP_TILE, JS_TP_TILE + f"${{state.attnMode !== 'standard' ? '<p class=\"sub\">~' + {JS_DECODE_ESTIMATE} + ' tok/s per user (rough)</p>' : ''}}", 1)]
S["R4 js: the same leak only when the state carries a catalog slug (gpuKey)"] = [
    (INDEX_HTML, JS_TP_TILE, JS_TP_TILE + f"${{state.gpuKey ? '<p class=\"sub\">~' + {JS_DECODE_ESTIMATE} + ' tok/s per user (rough)</p>' : ''}}", 1)]
S["R5 js: the 'FP8 KV cache' badge withheld for a card without constants"] = [
    (INDEX_HTML, FP8KV_BADGE, FP8KV_BADGE.replace("if (state.kvBytesPerValue < 2)", "if (state.kvBytesPerValue < 2 && computed.throughputModelled)"), 1)]
S["R6 js: the comparison card withholds the 'KV dtype' row for a single-device card without constants"] = [
    (INDEX_HTML, "    html += `<div class=\"row\"><span class=\"label\">KV dtype</span><span class=\"val\">${s.kvBytesPerValue < 2 ? 'FP8' : 'BF16'}</span></div>`;\n",
...
S["Q13 py: PDF prints the figure only when an FP8 KV cache is selected"] = [
    (REPORT_PY, PY_EXPLAIN, PY_EXPLAIN + "            if cfg.get(\"kv_bpp\", 2) < 2:\n                story.append(Paragraph(f\"Rough decode estimate: ~{c['single_tok'] or 0} tokens/sec per user\", self.styles[\"Small\"]))\n", 1)]
S["Q14 py: PDF prints the figure only for MLA or SWA attention"] = [
    (REPORT_PY, PY_EXPLAIN, PY_EXPLAIN + "            if cfg.get(\"attn\", \"standard\") != \"standard\":\n                story.append(Paragraph(f\"Rough decode estimate: ~{c['single_tok'] or 0} tokens/sec per user\", self.styles[\"Small\"]))\n", 1)]
S["Q15 py: the PDF's max-context row withheld for a single-device card without constants"] = [
    (REPORT_PY, "            [\"Max context (1 user, 90% util)\", fmt_k(c[\"max_ctx_1\"]) + \" tokens\"],\n",
