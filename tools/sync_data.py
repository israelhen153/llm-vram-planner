...
# stops this script instead of reaching the engines as a field nobody decided
# to empty. Both engines read null as "no confirmed hourly price", never as $0.
GPU_NULLABLE = ("hyper", "spec", "spot")
...
def row_fields(source, row_key, row, fields, optional, value, key, sep,
    ...
    in what order, which may be absent, and which may be null — is decided by
    the caller's field lists, so two renderers over the same table cannot
    disagree about them. `null` is the target language's literal for a field
    listed in `nullable`; a null anywhere else is refused, naming the field.
    """
    ...
    nulls = [f for f in names if row[f] is None and f not in nullable]
    if nulls:
        raise SystemExit(
            f"{source}: row {row_key!r} has null in {nulls}. Only {list(nullable) or 'no field'} "
            "may be null — a price tier with no confirmed hourly price."
        )
    return sep.join(f"{key(f)}{null if row[f] is None else value(row[f])}" for f in names)


def js_block(tag, source, var, rows, fields, optional, nullable=()):
    ...
    for key, row in rows.items():
        body = row_fields(source, key, row, fields, optional,
                          js_value, lambda f: f"{f}:", ", ", nullable, "null")
...
def render_gpu_js(gpus):
    return js_block("GPU_TABLE", "data/gpus.json", "GPU_TABLE",
                    gpus, GPU_FIELDS, GPU_OPTIONAL, GPU_NULLABLE)
...
def render_gpu_py(gpus):
    ...
    for key, g in gpus.items():
        body = row_fields(source, key, g, GPU_FIELDS, GPU_OPTIONAL,
                          py_value, lambda f: f'"{f}":', ",", GPU_NULLABLE, "None")
